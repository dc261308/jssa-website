...
# ----------------------------------------------------------------------------
# MEMBERSHIP MATRIX (read-only board reference)
# ----------------------------------------------------------------------------
# The full membership database — name, contact info, emergency contact, and the
# year-by-year membership history — lives in the registration/payment
...
_MATRIX_TTL = 120  # 2 min — board reference, freshness matters


def membership_matrix():
    """Read-only snapshot of the Membership Matrix for the board portal.
    Returns {'headers': [str, ...], 'rows': [[str, ...], ...], 'count': int}.
    Only columns that have a header are included (the matrix has many blank
    spacer columns), and blank member rows are skipped. Returns empty lists if
    the sheet isn't configured or can't be read."""
    now = time.time()
    with _lock:
        if _matrix_cache["data"] is not None and now - _matrix_cache["ts"] < _MATRIX_TTL:
            return _matrix_cache["data"]
    blank = {"headers": [], "rows": [], "count": 0}
    try:
        if not (MATRIX_SHEET_ID and _SA_JSON):
            return blank
        import gspread
        from google.oauth2.service_account import Credentials
        info = json.loads(_SA_JSON)
        ...
        creds = Credentials.from_service_account_info(info, scopes=scopes)
        gc = gspread.authorize(creds)
        sh = gc.open_by_key(MATRIX_SHEET_ID)
        try:
            ws = sh.worksheet(MATRIX_TAB)
        except gspread.WorksheetNotFound:
            return blank

        vals = ws.get_all_values()

        # Find the header row (the matrix has title rows above it). It's the row
        # that carries Email + First Name + Last Name.
        header_idx = None
        for i, r in enumerate(vals):
            low = [_clean(c).lower() for c in r]
            if "email" in low and "first name" in low and "last name" in low:
                header_idx = i
                break
        if header_idx is None:
            return blank

        header_row = vals[header_idx]
        # Keep only columns that actually have a header label (skip blank spacers).
        keep = [(ci, _clean(h)) for ci, h in enumerate(header_row) if _clean(h)]
        headers = [h for _, h in keep]
        email_ci = next((ci for ci, h in keep if h.lower() == "email"), None)

        rows = []
        for r in vals[header_idx + 1:]:
            # Skip rows with no email — those are blank/spacer rows.
            if email_ci is None or email_ci >= len(r) or not _clean(r[email_ci]):
                continue
            rows.append([_clean(r[ci]) if ci < len(r) else "" for ci, _ in keep])

        result = {"headers": headers, "rows": rows, "count": len(rows)}
        with _lock:
            _matrix_cache["data"] = result
            _matrix_cache["ts"] = now
        return result
    except Exception:
        return _matrix_cache["data"] or blank
# ----------------------------------------------------------------------------
